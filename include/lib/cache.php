...
class Cache {
    ...
    public function readCache($cacheName) {
        ...
        if($cacheName == 'navi'){
            $naviModel = new Navi_Model();

            $res = $naviModel->getNavis();
            $list =  [];
            foreach($res as &$val){
                if($val['hide'] == 'n') {
                    $list[] = $val;
    ...
    private function mc_sort() {
        ...
        $sort_cache = $Sort_Model->getSorts('goods');
        $cacheData = serialize($sort_cache);
        $this->cacheWrite($cacheData, 'sort');
    }
    private function mc_blog_sort() {
        $Sort_Model = new Sort_Model();
        $sort_cache = $Sort_Model->getSorts('blog');
        $cacheData = serialize($sort_cache);
        $this->cacheWrite($cacheData, 'blog_sort');
    }
    ...
    private function mc_navi() {
        $navi_cache = [];
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "navi WHERE hide='n' ORDER BY pid ASC, taxis DESC");
