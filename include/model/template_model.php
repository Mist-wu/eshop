...
class Template_Model {
    ...
    function getCurrentTemplate(){
        $nonce_template = isMobile() ? Option::get('nonce_templet_tel') : Option::get('nonce_templet');
        
        if (empty($nonce_template)) {
            return null;
        }
        
        if (!file_exists(TPLS_PATH . $nonce_template . '/header.php')) {
            return null;
        }
        
        $tplData = implode('', @file(TPLS_PATH . $nonce_template . '/header.php'));
        ...
        preg_match("/HeaderAndFooter:(.*)/i", $tplData, $headerAndFooter);
        
        $tplInfo = [
            'tplfile'    => $nonce_template,
            'tplname'    => !empty($tplName[1]) ? subString(strip_tags(trim($tplName[1])), 0, 16) : $nonce_template,
            ...
            'header_and_footer' => !empty($headerAndFooter[1]) ? $headerAndFooter[1] : 'common',
        ];
        
        $previewPath = TPLS_PATH . $nonce_template . '/preview.jpg';
        $tplInfo['preview'] = file_exists($previewPath) ? (TPLS_URL . $nonce_template . '/preview.jpg') : './views/images/theme.png';
