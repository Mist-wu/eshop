...
var myBlog = {
    ...
    tocSetArray: function () {  // 设置toc的数组内填数据
        var $titles = $("#eshopEchoLog h1,h2,h3,h4,h5,h6:eq(0)")

        for (var i = 0; i < $titles.length; i++) {  // 将标签数据依次存入数组
            let $tit = $("#eshopEchoLog [toc-date='title']:eq(" + i + ")")
     ...
     */
    tocAnalyse: function () {
        var tocFlag = document.querySelector("#eshopEchoLog p")

        if ($("#eshopEchoLog").length === 0) return  // 不在阅读页面  退出
        if (!this.tocFlag.test($('#eshopEchoLog').html().substring(0, 30))) return  // 未声明 toc 标签，退出
