...
$sql = "SELECT content, create_time AS deliver_time
            FROM {$db_prefix}stock_usage
            WHERE order_list_id = {$viewChild['id']} AND stock_id = 0
            ORDER BY id ASC
            LIMIT 500";
    $secrets = $db->fetch_all($sql);

    $countRow = $db->once_fetch_array("SELECT COUNT(*) AS total FROM {$db_prefix}stock_usage WHERE order_list_id = {$viewChild['id']} AND stock_id = 0");
    ...
    <div class="order-detail-page">
        ...
        <section class="secret-panel">
            <div class="secret-header">
                <div>
                    <h3 style="margin: 0 0 6px; font-size: 16px;">发货信息</h3>
                ...
                <?php if ($secretCount > 0): ?>
                    <div class="secret-actions">
                        <button class="detail-btn" id="copyAllBtn"><i class="fa fa-copy"></i> 复制全部</button>
                        <button class="detail-btn primary" id="downloadBtn"><i class="fa fa-download"></i> 下载内容</button>
                    </div>
                <?php endif; ?>
            </div>
            ...
            <?php else: ?>
                <div class="empty-state">
                    ...
                    <?php if ($orderStatus === 0): ?>
                        <p>订单尚未支付，支付完成后将显示发货信息。</p>
                    <?php elseif ($orderStatus === 1): ?>
                        <p>订单已支付，正在等待发货。</p>
                    <?php else: ?>
                        <p>暂无发货信息，如有问题请联系客服。</p>
                    <?php endif; ?>
                </div>
            <?php endif; ?>
        </section>
    ...
    (function () {
        ...
        if (copyAllBtn) {
            copyAllBtn.addEventListener('click', function() {
                ...
                copyToClipboard(allContent).then(function() {
                    self.innerHTML = '<i class="fa fa-check"></i> 已复制';
                    if (window.layui && window.layui.layer) {
                        layui.layer.msg('已复制 ' + allSecrets.length + ' 条内容');
                    }
        ...
        if (downloadBtn) {
            downloadBtn.addEventListener('click', function() {
                ...
                a.download = '发货内容_' + filenameSeed + '.txt';
