...
if (empty($action)) {
    ...
    $br = '<a href="./">控制台</a><a href="./setting.php">系统管理</a><a><cite>基础设置</cite></a>';
...
if ($action == 'index') {
    ...
    $br = '<a href="./">控制台</a><a href="./setting.php">系统管理</a><a><cite>首页设置</cite></a>';
...
if ($action == 'save') {
    ...
    $getData = [
        ...
        'home_icon'           => Input::postStrVar('home_icon'),
...
if ($action == 'seo') {
    ...
    $br = '<a href="./">控制台</a><a href="./setting.php">系统管理</a><a><cite>SEO设置</cite></a>';
...
if ($action == 'mail') {
    ...
    $br = '<a href="./">控制台</a><a href="./setting.php">系统管理</a><a><cite>邮箱设置</cite></a>';
...
if ($action == 'user') {
    ...
    $register_email_switch = isset($options_cache['register_email_switch']) ? $options_cache['register_email_switch'] : 'y';
    $register_tel_switch = isset($options_cache['register_tel_switch']) ? $options_cache['register_tel_switch'] : 'y';
    ...
    $br = '<a href="./">控制台</a><a href="./setting.php">系统管理</a><a><cite>用户设置</cite></a>';
...
if ($action == 'shop') {
    ...
    $guest_query_contact_placeholder_order = $guest_query_contact_placeholder_order ?? '请输入您的联系方式';
    ...
    $guest_query_password_placeholder_query = $guest_query_password_placeholder_query ?? '请输入您设置的订单密码';

    $br = '<a href="./">控制台</a><a href="./setting.php">系统管理</a><a><cite>商城配置</cite></a>';
    include View::getAdmView('header');
    require_once(View::getAdmView('setting_shop'));
    include View::getAdmView('footer');
    View::output();
}
...
if ($action == 'shop_save') {
    ...
    // 游客查单模式验证：至少开启一项
    $contactSwitch = $_config['guest_query_contact_switch'] ?? 'n';
    $passwordSwitch = $_config['guest_query_password_switch'] ?? 'n';
    if ($contactSwitch === 'n' && $passwordSwitch === 'n') {
    ...
    $getData = [
        ...
        'guest_query_contact_placeholder_order' => $_config['guest_query_contact_placeholder_order'] ?? '请输入您的联系方式',
        'guest_query_contact_placeholder_query' => $_config['guest_query_contact_placeholder_query'] ?? '请输入您下单时填写的联系方式',
        'guest_query_password_switch' => $passwordSwitch,
        'guest_query_password_placeholder_order' => $_config['guest_query_password_placeholder_order'] ?? '请设置订单密码',
        'guest_query_password_placeholder_query' => $_config['guest_query_password_placeholder_query'] ?? '请输入您设置的订单密码',
    ];
    foreach ($getData as $key => $val) {
        Option::updateOption($key, $val);
    }
