...
class SkuTable {
    ...
    options = {
        ...
        singleSkuTableConfig: {
            thead: [
                ...
                {title: '库存', icon: 'icon-edit'},
                {title: '库存', icon: 'icon-edit'},
                // {title: '库存', icon: 'layui-icon-cols'},
                {title: '状态', icon: ''},
            ],
            tbody: [
                {type: 'input', field: 'price', value: '0', reqtext: '销售价不能为空'},
                {type: 'input', field: 'market_price', value: '0', reqtext: '市场价不能为空'},
                {type: 'input', field: 'cost_price', value: '0', reqtext: '成本价不能为空'},
                {type: 'input', field: 'stock', value: '0', reqtext: '库存不能为空'},
                {type: 'sdk', field: 'stock_sdk', value: '0', reqtext: '库存不能为空'},
        ...
        multipleSkuTableConfig: {
            ...
            tbody: [
                ...
                {type: 'sdk', field: 'stock_sdk', value: '0', verify: 'required|number', reqtext: '库存不能为空'},
                {
                    type: 'select',
                    field: 'status',
    ...
    constructor(options) {
        ...
        if (this.options.skuDataUrl) {
            Util.request.get({url: this.options.skuDataUrl, data: {goods_id: this.data.goods_id}}, (res) => {
    ...
    listen() {
         ...
         * 监听所选规格值的变化
         */
        form.on('change', 'input[lay-filter="fairy-spec-filter"]', function (data) {
            var specData = [];
        ...
        $(document).on('click', `#${this.options.specTableElemId} .fairy-spec-create`, function () {
            layer.prompt({title: '规格'}, function (value, index, elem) {
                Util.request.post(
                    {url: that.options.specCreateUrl, data: {title: value, goods_type_id: that.data.productTypeId}},
                    function (res) {
         ...
         * 监听删除规格/规格值
         */
        $(document).on('click', `#${this.options.specTableElemId} i.listen-delete`, function () {
    ...
    renderFormItem(label, content, target) {
        var html = '';
        if(this.options.operate == 'stock' && (target == 'fairy-is-attribute' || target == 'fairy-product-type' || target == 'fairy-attribute-table' || target == 'fairy-spec-table')){
    ...
    renderIsAttribute(type) {
        ...
        html += '<label class="layui-form-label">规格类型</label>';
        html += '<div class="layui-input-block">';
        html += `<input type="radio" name="is_sku" value="n" lay-filter="fairy-is-attribute" ${type == '0' ? 'checked' : ''} title="单规格">`;
        html += `<input type="radio" name="is_sku" value="y" lay-filter="fairy-is-attribute" ${type == '1' ? 'checked' : ''} title="多规格">`;
        html += '</div>';
        html += '</div>';
        // this.renderFormItem('商品规格', html, this.options.isAttributeElemId);
        this.renderFormItem('', html, this.options.isAttributeElemId);
    ...
    renderProductType() {
        Util.request.get({url: this.options.productTypeUrl}, (res) => {
            var html = '<div class="layui-form-item">';
            html += '<label class="layui-form-label">规格模板</label><div class="layui-input-block">';
            html += `<select name="attr_id" lay-filter="fairy-product-type" id="${this.options.productTypeElemId}">`;
            html += '<option value="">请选择商品规格模板</option>';
            res.data.forEach((item) => {
                html += `<option value="${item.id}" ${this.data.productTypeId == item.id ? 'selected' : ''}>${item.title}</option>`;
            });
            html += '</select></div></div>';
            this.renderFormItem('', html, this.options.productTypeElemId);
            // this.renderFormItem('商品类型', html, this.options.productTypeElemId);
            if (this.data.productTypeId) {
                Util.request.get({url: this.options.attrSpecUrl, data: {goods_type_id: this.data.productTypeId, goods_id: this.data.goods_id}}, (res) => {
    ...
    renderSpecTable() {
        ...
        $.each(this.data.specData, function (index, item) {
            ...
            table += '<td>';
            table += '<div class="layui-form">';
            $.each(item.options, function (key, value) {
                table += `<input type="checkbox" title="${value.title}" lay-filter="fairy-spec-filter" value="${value.id}" ${item.value.includes(value.id) ? 'checked' : ''} /></label>`;
            });
            table += '</div>';
    ...
    renderSingleSkuTable() {
        var that = this,
            table = `<div style="overflow: auto;"><table class="layui-table" id="${this.options.skuTableElemId}">`;
        table += '<thead>';
        table += '<tr>';
    ...
    renderMultipleSkuTable() {
        var that = this, table = `<div style="overflow: auto;"><table class="layui-table" id="${this.options.skuTableElemId}">`;
        ...
        if ($(`#${this.options.specTableElemId} tbody input[type=checkbox]:checked`).length) {
            ...
            table += '<colgroup>' + '<col width="100">'.repeat(prependThead.length) + '</colgroup>';
            ...
            if (prependThead.length > 0) {
                ...
                theadTr += prependThead.map(function (t, i, a) {
                    return '<th class="fairy-spec-name" style="width: auto; white-space: nowrap;">' + t + '</th>';
                }).join('');

                this.options.multipleSkuTableConfig.thead.forEach(function (item) {
                    theadTr += '<th style="width: 200px;">' + item.title + (item.icon ? ' <i class="icon ' + item.icon + '"></i>' : '') + '</th>';
                });
                theadTr += '</tr>';
                ...
                that.options.multipleSkuTableConfig.tbody.forEach(function (c) {
                    ...
                    switch (c.type) {
                        ...
                        case "select":
                            ...
                            break;
                        case "sdk":
                            tr += '<td><button data-id="' + item.id + '" id="' + that.makeSkuName(item, c) + '" class="add-sku-stock-btn" type="button">添加卡密</button><textarea style="display: none;" type="text" name="' + that.makeSkuName(item, c) + '"></textarea></td>'
                            break;
                        case "number":
                            console.log(that.data.skuData);
                            console.log(that.makeSkuName(item, c))
                            console.log(item)
                            console.log(c)
                            tr += '<td><input type="number" name="' + that.makeSkuName(item, c) + '" value="' + (that.data.skuData[that.makeSkuName(item, c)] !== undefined ? that.data.skuData[that.makeSkuName(item, c)] : '') + '" class="layui-input" lay-verify="' + c.verify + '" lay-reqtext="' + c.reqtext + '" /></td>';
                            break;
                        case "input":
                        default:
                            console.log(that.data.skuData[that.makeSkuName(item, c)]);
                            console.log(c.value)
                            tr += '<td><input type="text" name="' + that.makeSkuName(item, c) + '" value="' + (that.data.skuData[that.makeSkuName(item, c)] !== undefined ? that.data.skuData[that.makeSkuName(item, c)] : '') + '" class="layui-input" lay-verify="' + c.verify + '" lay-reqtext="' + c.reqtext + '" /></td>';
                            break;
                    }
                });
                tr += '</tr>';
    ...
    getFormSkuData() {
        var skuData = {};
        var formFilter = this.getFormFilter(); // 调用之前定义的 getFormFilter 方法
        var fariyForm = $('form.fairy-form[lay-filter="' + formFilter + '"]');

        fariyForm.find('input, select, textarea').each(function() {
            var key = $(this).attr('name');
            if (key && key.startsWith('skus')) {
                var value = $(this).val();
                skuData[key] = value;
            }
        });
...
$('body').on('change', 'input[lay-filter="fairy-is-attribute"]', function () {
    let is_sku = $(this).val() == 'y' ? 1 : 0;
    skuTableObj.options.mode = is_sku;
    skuTableObj.render();
});
...
layui.use(function(){
    ...
    form.on('select(fairy-product-type)', function(data){
        ...
        skuTableObj.data.productTypeId = value;
        if (value === '') {
            skuTableObj.resetRender([skuTableObj.options.attributeTableElemId, skuTableObj.options.specTableElemId, skuTableObj.options.skuTableElemId]);
            skuTableObj.data.attributeData = [];
            skuTableObj.data.specData = [];
        } else {
            Util.request.get({url: skuTableObj.options.attrSpecUrl, data: {goods_type_id: value}}, (res) => {
                skuTableObj.resetRender([skuTableObj.options.attributeTableElemId, skuTableObj.options.specTableElemId, skuTableObj.options.skuTableElemId]);
                skuTableObj.data.attributeData = res.data.attribute;
                skuTableObj.data.specData = res.data.spec;
                skuTableObj.renderSpecTable();
                skuTableObj.renderMultipleSkuTable();
            });
        }
    });
    ...
    form.on('checkbox(fairy-spec-filter)', function (data) {
        var specData = [];
        $.each($(`#${skuTableObj.options.specTableElemId} tbody tr`), function () {
            var options = [], value = [];
            $.each($(this).find('input[type=checkbox]'), function () {
                $(this).is(':checked') && value.push($(this).val());
                options.push({id: $(this).val(), title: $(this).attr('title')});
        ...
        skuTableObj.data.specData = specData;
        skuTableObj.data.skuData = $.extend(skuTableObj.data.skuData, skuTableObj.getFormSkuData());
        skuTableObj.resetRender([skuTableObj.options.skuTableElemId]);
        skuTableObj.renderMultipleSkuTable();
    });
});